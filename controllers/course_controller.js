...
module.exports.createCourse=async function(req,res){
    let student=await Student.findById(req.params.id);
    let course=await Course.find({name:req.body.name,startDate:req.body.date});
    if(!course){
        course=await Course.create({name:req.body.name,startDate:req.body.date,completionScore:req.body.score});
